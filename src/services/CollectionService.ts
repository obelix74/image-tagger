import { Collection, CollectionRule, ImageMetadata, GeminiAnalysis, ImageExifMetadata } from '../types';
...
export class CollectionService {
   ...
   */
  static async createDefaultCollections(userId: number): Promise<void> {
   ...
   */
  static async createCollection(collection: Omit<Collection, 'id'>): Promise<number> {
    return DatabaseService.insertCollection(collection);
  }
  ...
  static async getUserCollections(userId: number): Promise<Collection[]> {
    return DatabaseService.getUserCollections(userId);
  }

  /**
   * Get a specific collection with image count
   */
  static async getCollection(collectionId: number): Promise<Collection | null> {
    return DatabaseService.getCollection(collectionId);
  }

  /**
   * Get images in a collection
   */
  static async getCollectionImages(collectionId: number): Promise<ImageMetadata[]> {
    const collection = await DatabaseService.getCollection(collectionId);
    if (!collection) return [];

    if (collection.type === 'manual') {
      return DatabaseService.getManualCollectionImages(collectionId);
    } else {
   ...
   */
  static async addImageToCollection(collectionId: number, imageId: number): Promise<void> {
    return DatabaseService.addImageToCollection(collectionId, imageId);
  }

  /**
   * Remove image from manual collection
   */
  static async removeImageFromCollection(collectionId: number, imageId: number): Promise<void> {
    return DatabaseService.removeImageFromCollection(collectionId, imageId);
  }

  /**
   * Update collection
   */
  static async updateCollection(collectionId: number, updates: Partial<Collection>): Promise<void> {
    return DatabaseService.updateCollection(collectionId, {
      ...updates,
      updatedAt: new Date().toISOString()
    });
  }
  ...
  static async deleteCollection(collectionId: number): Promise<void> {
    return DatabaseService.deleteCollection(collectionId);
  }

  /**
   * Evaluate smart collection rules and return matching images
   */
  private static async evaluateSmartCollection(collection: Collection): Promise<ImageMetadata[]> {
    ...
    return DatabaseService.executeSmartCollectionQuery(baseQuery, params);
  }

  /**
   * Build SQL condition for a collection rule
   */
  private static buildRuleCondition(rule: CollectionRule, params: any[]): string | null {
   ...
   */
  static async autoOrganizeImages(userId: number): Promise<void> {
    // Get all completed images for the user
    const { images } = await DatabaseService.getAllImages(undefined, undefined, userId);
    const completedImages = images.filter(img => img.status === 'completed');

    for (const image of completedImages) {
      try {
        const analysis = await DatabaseService.getAnalysis(image.id!);
        const metadata = await DatabaseService.getImageMetadata(image.id!);
   ...
   */
  private static async categorizeImageByContent(
    image: ImageMetadata, 
    ...
    for (const category of contentCategories) {
      const hasMatch = category.keywords.some(keyword => 
        keywords.some(imgKeyword => imgKeyword.toLowerCase().includes(keyword))
      );
   ...
   */
  private static async categorizeImageByLocation(
    image: ImageMetadata, 
   ...
   */
  private static async categorizeImageByCamera(
    image: ImageMetadata, 
    metadata: ImageExifMetadata, 
    userId: number
  ): Promise<void> {
   ...
   */
  private static async findOrCreateContentCollection(name: string, userId: number): Promise<Collection | null> {
   ...
   */
  private static async findOrCreateLocationCollection(location: string, userId: number): Promise<Collection | null> {
    const collections = await DatabaseService.getUserCollections(userId);
   ...
   */
  private static async findOrCreateCameraCollection(camera: string, userId: number): Promise<Collection | null> {
    const collections = await DatabaseService.getUserCollections(userId);
